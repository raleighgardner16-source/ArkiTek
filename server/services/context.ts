...
const detectCategoryForJudge = async (prompt: string, userId: string | null = null) => {
...
"${prompt}"`
  ...
  try {
    ...
    if (userId) {
      const responseTokens = extractTokensFromResponse(response.data, 'google') as { inputTokens?: number; outputTokens?: number } | null
      if (responseTokens) {
...
const summarizeJudgeResponse = async (judgeResponseText: string, userId: string | null = null) => {
...
const storeJudgeContext = async (userId: string, judgeResponse: string, originalPrompt: string | null = null) => {
  if (!userId) return
...
const storeModelContext = async (userId: string, modelName: string, modelResponse: string, originalPrompt: string | null = null) => {
