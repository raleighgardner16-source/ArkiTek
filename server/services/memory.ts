...
async function generateEmbedding(text: string, userId: string | null = null): Promise<number[] | null> {
...
function buildEmbeddingText(originalPrompt: string, responses: any[], summary: any, conversationTurns: any[]): string {
...
async function findRelevantContext(userId: string, currentPrompt: string, limit: number = 3, scoreThreshold: number = 0.75, targetModel: string | null = null): Promise<any[]> {
  ...
  try {
    ...
    const results = await db.conversationHistory.vectorSearch(queryEmbedding, userId, limit + 5, 50)
    ...
    const contexts = results
      ...
      .map((r: any) => {
        ...
        if (targetModel && r.responses && r.responses.length > 0) {
          const modelResponse = r.responses.find((resp: any) =>
        ...
        if (!snippet && r.summary?.text) {
          ...
          if (summaryStart !== -1) {
            ...
            const lines = summaryText.split('\n').filter((l: string) => l.trim() && !l.startsWith('**CONSENSUS'))
    ...
    console.log(`[Memory] Found ${contexts.length} relevant past conversations for user ${userId} (scores: ${contexts.map((c: any) => c.score?.toFixed(3)).join(', ')}, threshold: ${scoreThreshold}${targetModel ? `, targetModel: ${targetModel}` : ''})`)
    return contexts
  } catch (error: any) {
...
function formatMemoryContext(contexts: any[]): string {
