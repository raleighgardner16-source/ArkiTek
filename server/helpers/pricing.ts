import { estimateTokensFallback } from './tokenCounters.js'

const buildTokenBreakdown = (userQuery: string, sourceContent: string, totalInputTokens: number) => {
...
interface UserForPlan {
  plan?: string
  subscriptionStatus?: string
  stripeSubscriptionId?: string
}

const getPlanAllocation = (user: UserForPlan): number => {
...
interface ModelCostPricing {
  input: number
  output: number
  per: number
}

const calculateModelCost = (modelKey: string, inputTokens: number, outputTokens: number, pricing: Record<string, ModelCostPricing>): number => {
...
const calculateSerperQueryCost = (queryCount: number): number => {
