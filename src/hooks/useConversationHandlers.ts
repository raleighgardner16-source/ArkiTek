...
export function useConversationHandlers({ isLoading, isGeneratingSummary }: UseConversationHandlersProps) {
  // ---- Store selectors ---- //
  ...
  const [summaryConvoSources, setSummaryConvoSources] = useState<Record<string, any>>({})
  const [showSummaryConvoSources, setShowSummaryConvoSources] = useState<Record<string, boolean>>({})
  const [showCouncilColumnSources, setShowCouncilColumnSources] = useState<Record<string, boolean>>({})
  ...
  const [singleModelConvoHistory, setSingleModelConvoHistory] = useState<any[]>([])
  const [singleModelInitialSources, setSingleModelInitialSources] = useState<any[]>([])
  const [singleConvoSources, setSingleConvoSources] = useState<Record<string, any>>({})
  const [showSingleConvoSources, setShowSingleConvoSources] = useState<Record<string, boolean>>({})

  // Council column conversation state
  const [councilColumnConvoInputs, setCouncilColumnConvoInputs] = useState<Record<string, string>>({})
  const [councilColumnConvoSending, setCouncilColumnConvoSending] = useState<Record<string, boolean>>({})
  const [councilColumnConvoSearching, setCouncilColumnConvoSearching] = useState<Record<string, boolean>>({})
  const [councilColumnConvoSources, setCouncilColumnConvoSources] = useState<Record<string, any>>({})
  const [showCouncilColumnConvoSources, setShowCouncilColumnConvoSources] = useState<Record<string, boolean>>({})
  ...
  // ---- Refs ---- //
  const convoTextareaRef = useRef<HTMLTextAreaElement | null>(null)
  const singleConvoTextareaRef = useRef<HTMLTextAreaElement | null>(null)
  const singleConvoAbortControllerRef = useRef<AbortController | null>(null)
      ...
      if (activeHistoryId && currentUser?.id) {
        ...
        if (latestTurn && latestTurn.assistant) {
          api.post('/history/update-conversation', {
            ...
            .catch((err: any) => console.error('[History] Error updating judge conversation turn:', err.message))
        }
      }
    } catch (error: any) {
    ...
    try {
      ...
      if (activeHistoryId && currentUser?.id) {
        api.post('/history/update-conversation', {
          ...
          .catch((err: any) => console.error('[History] Error updating single model conversation turn:', err.message))
      }
    } catch (error: any) {
      if (error.name === 'AbortError') {
  ...
  const handleSendCouncilColumnConvo = async (response: any) => {
      ...
      if (activeHistoryId && currentUser?.id) {
        ...
        if (assistantText) {
          api.post('/history/update-conversation', {
            ...
            .catch((err: any) => console.error('[History] Error updating council column conversation turn:', err.message))
        }
      }
    } catch (error: any) {
      if (error.name === 'AbortError') {
  ...
  const handleSendCouncilFollowUp = async () => {
    ...
    const activeResponses = responses.filter((r: any) => !r.error)
    if (activeResponses.length === 0) return

    setCouncilFollowUpInput('')
    setCouncilFollowUpSending(true)

    api.post('/conversation/track-follow-up', {
      userMessage: userMsg,
    }).catch((err: any) => console.error('[Council Follow-Up] Error tracking prompt:', err.message))

    activeResponses.forEach((response: any) => {
      const responseId = response.id
      setCouncilColumnConvoHistory(prev => ({
        ...prev,
        [responseId]: [...(prev[responseId] || []), { user: userMsg, assistant: '', timestamp: Date.now() }]
    ...
    const promises = activeResponses.map(async (response: any) => {
      const responseId = response.id
      const modelName = response.modelName || response.actualModelName
      if (!modelName) return
      const prevTurnCount = (councilColumnConvoHistory[responseId] || []).length
        ...
        if (activeHistoryId && currentUser?.id) {
          ...
          if (assistantText) {
            api.post('/history/update-conversation', {
              ...
              .catch((err: any) => console.error('[History] Error updating council follow-up turn:', err.message))
          }
        }
      } catch (error: any) {
        if (error.name !== 'AbortError') {
  ...
  const getProviderDisplayName = (modelName: string): string => {
  ...
  const responsesWithText = responses.filter((r: any) => r.text?.length > 0 && !r.error)
  const councilDisplayResponses = responses.filter((r: any) => !r.error)
  const councilColumnCount = councilDisplayResponses.length
  const primaryResponse = responses.find((r: any) => !r.error) || null
  ...
  const hasCouncilResponsesForView = responses.filter((r: any) => !r.error).length >= 2
