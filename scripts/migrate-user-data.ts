...
async function migrate(): Promise<void> {
  ...
  try {
    ...
    if (!oldUsage) {
      ...
      const { _id: _oldId, updatedAt: _oldUpdated, ...oldData } = oldUsage as any
      const { _id: _newId, updatedAt: _newUpdated, ...newData } = (newUsage || {}) as any
      ...
      for (const key of ['promptHistory', 'earnedBadges']) {
        ...
        if (key === 'earnedBadges') {
          ...
          const seen = new Set()
          merged[key] = [...oldArr, ...newArr].filter((b: any) => {
    ...
    if (!oldStats) {
      ...
      const { _id: _oldId2, ...oldStatsData } = oldStats as any
      const { _id: _newId2, ...newStatsData } = (newStats || {}) as any
